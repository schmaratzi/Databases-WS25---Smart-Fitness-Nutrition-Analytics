...
SELECT
p.gender, COUNT(*) as n_probands, AVG(n.meal_frequency) as avg_meal_frequency, AVG(w.calories_burned) as avg_calories_burned
FROM fitness.proband p
JOIN fitness.nutrition n ON p.proband_id = n.proband_id
JOIN fitness.workout w ON w.proband_id = p.proband_id
